...
export class Renderer {
    ...
    drawMilitarySymbol(unit, x, y, width, height) {
        ...
        if (isCommandFamily) {
            ...
            const flagTop = y - height * 0.25;
            const flagMid = y - height * 0.07;
            const flagRight = x + width * 0.08;
            ctx.beginPath();
            ctx.moveTo(left, y + height * 0.26);
            ctx.lineTo(left, flagTop);
            ctx.lineTo(flagRight, y - height * 0.18);
            ctx.lineTo(left, flagMid);
            ctx.stroke();
            const rankText = `${echelon} ${name}`;
            let stars = 1;
            if (/front|方面军|armygroup|集团军群/.test(rankText)) stars = 3;
            else if (/army|集团军/.test(rankText) && !/corps|军级|装甲军|步兵军|骑兵军|机械化军/.test(rankText)) stars = 2;
            else if (/corps|军级|装甲军|步兵军|骑兵军|机械化军/.test(rankText)) stars = 1;
            else if (/division|师/.test(rankText)) stars = 1;
            const sx = x + width * 0.06;
            const sy = y + height * 0.08;
            const radius = Math.max(1.5, height * 0.055);
            for (let i = 0; i < stars; i++) {
                this.drawPLAStar(sx + i * width * 0.14, sy, radius * 1.75);
            }
            if (isGuard) {
                ctx.beginPath();
                ctx.arc(x + width * 0.28, y - height * 0.19, Math.max(1.2, height * 0.045), 0, Math.PI * 2);
                ctx.fill();
            }
        } else if (isEngineer) {
            ctx.beginPath();
            ...
            ctx.beginPath();
            ctx.moveTo(x, y - height * 0.23);
            ctx.lineTo(x, y + height * 0.23);
            ctx.stroke();
            ctx.beginPath();
            ctx.arc(x, y, height * 0.11, 0, Math.PI * 2);
            ctx.fill();
        } else if (isCavalry) {
            ctx.beginPath();
