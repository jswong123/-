...
export class Renderer {
    ...
    compactUnitName(name = "") {
        ...
        // 地图中景只保留足以区分隶属关系的番号。
        text = text.replace(/装甲团第(\d+)装甲营/g, "装团$1营");
        text = text.replace(/摩托化步兵团第(\d+)摩步营/g, "摩团$1营");
        text = text.replace(/摩步团第(\d+)摩步营/g, "摩团$1营");
        text = text.replace(/步兵团第(\d+)步兵营/g, "步团$1营");
        text = text.replace(/炮兵团第(\d+)炮兵营/g, "炮团$1营");
        text = text.replace(/师部第(\d+)警卫连/g, "师警$1连");
        text = text.replace(/团部警卫连/g, "团警连");
    ...
    // ========================================================
    // 安全绘制单个图层：单一图层异常不再中断后续地名和单位绘制
    // ========================================================
    safeDrawLayer(name, drawFn) {
        try {
            drawFn();
        } catch (error) {
            console.error(`[渲染系统] ${name} 图层绘制失败：`, error);
        }
    }
    // ========================================================
    // 总渲染
    ...
    render(units = []) {
        this.clear();
        this.safeDrawLayer("地形", () => this.drawTerrain());
        this.safeDrawLayer("道路", () => this.drawRoads());
        this.safeDrawLayer("铁路", () => this.drawRailways());
        this.safeDrawLayer("河流", () => this.drawRivers());
        this.safeDrawLayer("防御工事", () => this.drawFortifications());
        this.safeDrawLayer("城镇与地名", () => this.drawSettlements());
        this.safeDrawLayer("移动范围", () => this.drawMovementRange());
        this.safeDrawLayer("军事单位", () => this.drawUnits(units));
    }
}
