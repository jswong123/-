...
export class Renderer {
    ...
    // 单位名称压缩
    // ========================================================
    compactUnitName(value) {
        const text = String(value ?? "").trim();
        if (!text) return "";
        const maxChars = this.camera.zoom >= 1.35 ? 18 : 13;
        return text.length > maxChars ? `${text.slice(0, maxChars)}…` : text;
    }
    // ========================================================
    // 防御工事
    // 兼容 fortifications / forts / defenses / defensiveWorks
    // 以及数组、Map、普通对象三种数据结构。
    // ========================================================
    drawFortifications() {
        const ctx = this.ctx;
        const source =
            ...
            this.world?.forts ??
            this.world?.defenses ??
            this.world?.defensiveWorks ??
            [];
        let entries = [];
        if (Array.isArray(source)) {
            entries = source;
        } else if (source instanceof Map) {
            entries = Array.from(source.entries()).map(([key, value]) => {
        ...
        const parseKey = (key) => {
            ...
            const match = key.match(/(-?\d+)\s*[,;:_|]\s*(-?\d+)/);
        ...
        ctx.lineCap = "round";
        ctx.lineJoin = "round";
        for (const fort of entries) {
            if (!fort) continue;
            let q = Number(fort.q ?? fort.hexQ ?? fort.col ?? fort.x);
            let r = Number(fort.r ?? fort.hexR ?? fort.row ?? fort.y);
            if (!Number.isFinite(q) || !Number.isFinite(r)) {
                const parsed = parseKey(fort._key ?? fort.key ?? fort.hex);
                if (!parsed) continue;
                q = parsed.q;
                r = parsed.r;
            }
            ...
            const level = Math.max(
                ...
                Math.min(
                    5,
                    Number(fort.level ?? fort.strength ?? fort.value ?? 1) || 1
                )
            );
            const half = size * 0.34;
            const amp = size * (0.08 + level * 0.008);
            ctx.beginPath();
            ctx.moveTo(p.x - half, p.y + amp);
            ctx.lineTo(p.x - half * 0.55, p.y - amp);
            ctx.lineTo(p.x - half * 0.12, p.y + amp);
            ctx.lineTo(p.x + half * 0.32, p.y - amp);
            ctx.lineTo(p.x + half, p.y + amp);
            ctx.stroke();
        }
        ctx.restore();
    }
