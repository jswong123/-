...
export class Renderer {
    ...
    drawMilitarySymbol(unit, x, y, width, height) {
        ...
        if (isHQ) {
            ...
            const left = x - width * 0.26;

            ctx.beginPath();

            ctx.moveTo(left, y + height * 0.25);

            ctx.lineTo(left, y - height * 0.28);
            ...
            const rank = String(unit.echelon ?? unit.level ?? unit.formation ?? "").toLowerCase();

            let stars = /front|armygroup|方面军|集团军群/.test(rank + name) ? 3 : /army|集团军/.test(rank + name) ? 2 : 1;

            const sx = x + width * 0.08;

            for (let i = 0; i < stars; i++) {

                ctx.beginPath();

                ctx.arc(sx + i * width * 0.14, y + height * 0.07, Math.max(1.5, height * 0.055), 0, Math.PI * 2);
