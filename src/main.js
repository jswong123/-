...
// ============================================================
import {
import { FortificationSystem } from "./systems/FortificationSystem.js";
    drawHexPath
} from "./Hex.js";
...
window.addEventListener("keydown", event => {
    ...
    const unit = selection?.selectedUnit;
    if (!unit) {
        console.log("[工事] 请先选择一个单位");
        return;
    }
    const playerSide = getPlayerSide();
    const currentSide = getCurrentSide();
    if (unit.faction !== playerSide || currentSide !== playerSide) {
        console.log("[工事] 只能在玩家行动阶段命令己方单位构筑");
        return;
    }
    undoSystem.push(
        { units, turnSystem, gameOver },
        `构筑工事：${unitName(unit)}`
    );
    const result = fortificationSystem.build(unit);
    if (!result?.success) {
        undoSystem.discardLast?.();
        updateSaveControls?.();
        console.log("[工事]", result?.reason ?? "构筑失败");
        return;
    }
    console.log(`[工事] ${unitName(unit)} 完成构筑，工事等级 ${result.level}`);
    updateSaveControls?.();
    render();
});
